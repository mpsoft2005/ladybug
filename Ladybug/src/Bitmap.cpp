...
void Bitmap::SetPixel(int x, int y, const Color& color)
{
	...
	c.r = (uint8_t)roundf(saturate(color.r) * 255);
	c.g = (uint8_t)roundf(saturate(color.g) * 255);
	c.b = (uint8_t)roundf(saturate(color.b) * 255);
	c.a = (uint8_t)roundf(saturate(color.a) * 255);
}
